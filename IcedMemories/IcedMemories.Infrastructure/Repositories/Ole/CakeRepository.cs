    ...
    public class CakeRepository : ICakeRepository
    {
        ...
        public virtual Task<IList<Cake>> GetCakesAsync()
        {
            ...
            return Task.Factory.StartNew(() =>
            {
                using (IDbConnection connection = CurrentContext.OpenConnection())
                    return (IList<Cake>)connection.Query<Cake>("select * FROM app_Cakes ORDER BY DateAdded DESC", new { }).ToList();
            });
        }
        ...
        public Task CreateAsync(Cake cake)
        {
            ...
            return Task.Factory.StartNew(() =>
            {
                ...
                using (IDbConnection connection = CurrentContext.OpenConnection())
                    connection.Execute("insert into app_Cakes(Id, DateAdded, Title, Description, ImageLink) values(@Id, @DateAdded, @Title, @Description, @ImageLink)", new { Id = cake.Id, DateAdded = cake.DateAdded.ToString("dd-MM-yyyy HH:mm:ss"), Title = cake.Title, Description = cake.Description, ImageLink = cake.ImageLink });
            });
        }
        ...
        public Task UpdateAsync(Cake cake)
        {
            ...
            {
                using (IDbConnection connection = CurrentContext.OpenConnection())
                    connection.Execute("update app_Cakes SET Title=@Title, Description=@Description, ImageLink=@ImageLink where Id = @Id", new { Title = cake.Title, Description = cake.Description, ImageLink = cake.ImageLink, Id = cake.Id});
            });
        }
