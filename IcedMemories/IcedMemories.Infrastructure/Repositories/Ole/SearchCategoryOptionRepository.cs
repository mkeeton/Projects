...
{
    public class SearchCategoryOptionRepository : ISearchCategoryOptionRepository
    {
          ...
          if (categoryOptionId == Guid.Empty)
            {
                return Task.Factory.StartNew(() =>
                {
                    return new SearchCategoryOption();
                });
            }
            else
            {
        ...
        public Task CreateAsync(SearchCategoryOption categoryOption)
        {
            ...
            return Task.Factory.StartNew(() =>
            {
                ...
                using (IDbConnection connection = CurrentContext.OpenConnection())
                    connection.Execute("insert into app_SearchCategoryOptions(Id, CategoryId, Name) values(@Id, @CategoryId, @Name)", new { Id = categoryOption.Id, CategoryId = categoryOption.CategoryId, Name = categoryOption.Name});
            });
        }
        ...
        public Task UpdateAsync(SearchCategoryOption categoryOption)
        {
            ...
            {
                using (IDbConnection connection = CurrentContext.OpenConnection())
                  connection.Execute("update app_SearchCategoryOptions SET CategoryId=@CategoryId, Name=@Name where Id = @Id", new { CategoryId = categoryOption.CategoryId, Name = categoryOption.Name, Id = categoryOption.Id });
            });
        }
    }
}
